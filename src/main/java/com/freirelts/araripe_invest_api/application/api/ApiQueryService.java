	...
	public record FundamentalResponse(UUID id, LocalDate referenceDate, PeriodType periodType, String source,
		...
		static FundamentalResponse from(FundamentalSnapshot snapshot) {
			return new FundamentalResponse(snapshot.getId(), snapshot.getReferenceDate(), snapshot.getPeriodType(),
					...
					snapshot.getOperatingMargin(), snapshot.getRoe(), snapshot.getRoa(), snapshot.getDebtToEquity(),
					snapshot.getRevenueGrowth(), snapshot.getEarningsGrowth(), snapshot.getAnnualRevenueGrowth(),
					snapshot.getQuarterlyRevenueGrowth(), snapshot.getAnnualEarningsGrowth(),
					snapshot.getQuarterlyEarningsGrowth(), snapshot.getEbitdaGrowth(), snapshot.getFreeCashflow(),
					snapshot.getOperatingCashflow(), snapshot.getNetDebt(), snapshot.getQualityStatus(),
					snapshot.getCalculationVersion());
		}
	}
